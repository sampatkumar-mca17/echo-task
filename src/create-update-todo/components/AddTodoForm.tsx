  ...
  return (
    <form onSubmit={handleSubmit} className='flex flex-col gap-8 p-2 justify-start items-start w-[100%] h-[calc(100vh-100px)]'>
        ...
        <CardContent>
            ...
            <select name="status" value={values.status} onChange={handleChange} className='p-4 cursor-pointer border border-solid rounded-sm'>
        ...
        <CardFooter>
            ...
            <Button className='cursor-pointer' variant="secondary" type="button" onClick={onCancel}>Cancel</Button>
            <Button className='cursor-pointer' variant="default" type="submit" disabled={apiDetails.isLoading}>
                {apiDetails.isLoading ? <Loader className='animate-spin'/> : (mode === 'update' ? 'Update': 'Add')}
            </Button>
            </div>
        </CardFooter>
        </Card>
